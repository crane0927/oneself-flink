 ...
 * className EopDataAnalysisProc
 * description EOP 数据解析（处理窗口） 统计 1 分钟窗口内，appId 和 apiId 相同的记录中 resultCode = 0 的占比
 * version 1.0
 */
public class EopDataAnalysisProc {
    private static final Logger log = LoggerFactory.getLogger(EopDataAnalysisProc.class);
    ...
    public static void main(String[] args) throws Exception {
        ...
        // 4. 分配时间戳和水位线，支持乱序数据
        SingleOutputStreamOperator<JsonNode> events = streamSource.assignTimestampsAndWatermarks(
                WatermarkStrategy.<JsonNode>forBoundedOutOfOrderness(Duration.ofSeconds(5)) // 允许 5 秒的乱序数据
        ...
        SingleOutputStreamOperator<JsonNode> process = events
                ...
                .window(TumblingProcessingTimeWindows.of(Duration.ofSeconds(10))) // 处理时间窗口
                .allowedLateness(Duration.ofSeconds(5))  // 允许迟到 5 秒的数据
